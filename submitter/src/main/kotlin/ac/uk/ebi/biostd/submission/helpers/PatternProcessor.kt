...
class PatternProcessor {
    ...
    fun generateAccNumber(accPattern: String, sequenceFunction: (String) -> Long): AccNumber {
        return getPrefixAccPattern(accPattern)
                .orElse { getPostfixAccPattern(accPattern) }
                .orElse { getPrefixPostfixPattern(accPattern) }
                .map { AccNumber(it, sequenceFunction(it.toString())) }
                .getOrElse { throw IllegalStateException() }
    }

    fun extractAccessNumber(accNo: String): AccNumber {
        return tryToGet(accNo, extractionPattern) {
