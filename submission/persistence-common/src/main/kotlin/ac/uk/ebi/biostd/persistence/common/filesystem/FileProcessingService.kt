...
class FileProcessingService {
    ...
    private fun processSubmission(processRequest: ProcessRequest): ExtSubmission {
    ...
    private fun processFileList(fileList: ExtFileList, processFile: (file: ExtFile) -> ExtFile
    ) = fileList.copy(files = fileList.files.map { processFile(it) })

    private fun processFiles(either: Either<ExtFile, ExtFileTable>, processFile: (file: ExtFile) -> ExtFile
    ) = either.bimap(
...
data class SubmissionConfig(
    val subFolder: File,
    val tempFolder: File, // Rename to target folder, this class does not need to know if a temporally one
    val filePermissions: Set<PosixFilePermission>,
    val folderPermissions: Set<PosixFilePermission>
)

private fun SubmissionConfig.copy(extFile: ExtFile): ExtFile {
...
private fun SubmissionConfig.move(extFile: ExtFile): ExtFile {
    val source = extFile.file
    val target = tempFolder.resolve(extFile.fileName)
