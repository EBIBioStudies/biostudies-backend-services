...
class SubmitPermissionTest(
    ...
    fun `4-6 submit with collection admin permission`() {
        ...
        assertThat(superUserWebClient.submitSingle(project, SubmissionFormat.TSV)).isSuccessful()
        superUserWebClient.grantCollectionPermission(ExistingUser.email, "TestCollection4", ADMIN.name)
    ...
    fun `4-7 resubmit with collection admin permission`() {
        ...
        assertThat(superUserWebClient.submitSingle(project, SubmissionFormat.TSV)).isSuccessful()
        superUserWebClient.grantCollectionPermission(ExistingUser.email, "TestCollection5", ADMIN.name)
    ...
    fun `4-10 not owner user resubmits with the update permission`() {
        ...
        superUserWebClient.grantSubmissionPermission(ExistingUser.email, "S-SBMT2", UPDATE.name)
        assertThat(regularUserWebClient.submitSingle(resubmission, SubmissionFormat.TSV)).isSuccessful()
    }
    ...
    private fun setAttachPermission(
        testUser: TestUser,
        collection: String,
    ) = superUserWebClient.grantCollectionPermission(testUser.email, collection, ATTACH.name)
