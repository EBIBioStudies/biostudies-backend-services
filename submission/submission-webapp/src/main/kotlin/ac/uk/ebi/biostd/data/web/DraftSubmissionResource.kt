...
class DraftSubmissionResource(private val draftSubService: DraftSubService) {
    @GetMapping(value = ["draft/submissions/{accNo}"], produces = [APPLICATION_JSON])
    @ResponseBody
    ...
    @GetMapping("draft/submissions", params = ["searchText"], produces = [APPLICATION_JSON])
    @ResponseBody
    ...
    @GetMapping("draft/submissions", params = ["!searchText"])
    @ResponseBody
    ...
    @DeleteMapping("draft/submissions/{accNo}")
