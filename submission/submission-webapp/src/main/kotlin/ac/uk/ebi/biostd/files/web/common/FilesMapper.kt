...
class FilesMapper {
    ...
    fun asUserFiles(fileSpec: FilesSpec) =
        fileSpec.files.map {
            UserFile(it.name, getPath(it, fileSpec.source, USER_FOLDER_NAME), it.length(), UserFileType.getType(it))
        }

    fun asGroupFiles(groupName: String, fileSpec: FilesSpec) =
        fileSpec.files.map {
            UserFile(it.name, getPath(it, fileSpec.source, "Groups/$groupName"), it.length(), UserFileType.getType(it))
        }
