...
class SubmitWebHandler(
    ...
    private fun buildRequest(request: ContentSubmitWebRequest): SubmissionRequest {
        ...
        val sourcesRequest = SourcesRequest(sub.accNo, sub.rootPath, request.submitter, request.files)
        val (fileSources, previousVersion) = webHandlerHelper.submissionSources(sourcesRequest)

        requireNotProcessing(previousVersion)
        val submission = withAttributes(submission(request.submission, request.format, fileSources), request.attrs)
    ...
    private fun buildRequest(request: FileSubmitWebRequest): SubmissionRequest {
        ...
        requireNotProcessing(previousVersion)
        val submission = withAttributes(submission(request.submission, fileSources), request.attrs)
    ...
    fun refreshSubmission(request: RefreshWebRequest): Submission {
        ...
        val sourcesRequest = SourcesRequest(submission.accNo, submission.rootPath)
        val (fileSources, previousVersion) = webHandlerHelper.submissionSources(sourcesRequest)
        requireNotProcessing(previousVersion)
