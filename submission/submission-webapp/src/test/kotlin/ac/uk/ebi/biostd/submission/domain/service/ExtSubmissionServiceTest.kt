...
class ExtSubmissionServiceTest(
    ...
    fun beforeEach() {
        ...
        every { submissionRepository.findExtByAccNo("S-TEST123") } returns extSubmission
        every { submissionRepository.findExtByAccNo("S-TEST124") } returns null
    ...
    @Test
    fun `find ext submission`() {
        assertThat(testInstance.findExtendedSubmission("S-TEST123")).isEqualTo(extSubmission)
        assertThat(testInstance.findExtendedSubmission("S-TEST124")).isEqualTo(null)
    }
    ...
    fun `submit extended`() {
        ...
        every { submissionSubmitter.submit(capture(saveRequest)) } returns extSubmission
        ...
        verify(exactly = 1) {
            ...
            submissionSubmitter.submit(saveRequest.captured)
        }
    }
    ...
    fun `submit extended async`() {
        ...
        every { submissionSubmitter.submitAsync(capture(saveRequest)) } returns extSubmission.copy(version = 2)
        every {
        ...
        verify(exactly = 1) {
            submissionSubmitter.submitAsync(saveRequest.captured)
            submissionRepository.existByAccNo("ArrayExpress")
            securityQueryService.existsByEmail("owner@email.org")
    ...
    fun `submit extended with file lists`(
        ...
        every { submissionSubmitter.submit(capture(saveRequest)) } returns submission
        every {
    ...
    fun `submit extended collection`() {
        ...
        every { submissionRepository.existByAccNo("ArrayExpress") } returns false
        every { submissionSubmitter.submit(capture(saveRequest)) } returns collection
