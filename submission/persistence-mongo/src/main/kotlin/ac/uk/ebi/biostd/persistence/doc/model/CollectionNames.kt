package ac.uk.ebi.biostd.persistence.doc.model

object CollectionNames {
    const val SUBS = "submissions"
    const val SUB_FILES = "submission_files"
    const val SUB_FILE_LIST_FILES = "file_list_files"
    const val SUB_LINK_LIST_LINKS = "link_list_links"
    const val SUB_STATS = "submission_stats"

    const val SUB_RQT = "submission_requests"
    const val SUB_RQT_ARCHIVE = "submission_requests_archive"
    const val SUB_RQT_FILES = "submission_request_files"
    const val SUB_RQT_FILES_ARCHIVE = "submission_request_files_archive"

    const val PMC_SUBMISSIONS = "pmc_submissions"
}
