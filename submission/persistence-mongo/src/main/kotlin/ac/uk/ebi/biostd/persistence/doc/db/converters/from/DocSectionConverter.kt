...
class DocSectionConverter(
    ...
    private fun toEitherLinks(doc: Document) =
        when (val clazz = doc.getString(CLASS_FIELD)) {
            ...
            else -> error("Expecting $clazz to be one of [$DOC_LINK_CLASS , $DOC_LINK_TABLE_CLASS]")
        }

    private fun toEitherFiles(doc: Document) =
        when (val clazz = doc.getString(CLASS_FIELD)) {
    ...
    private fun toEitherSections(doc: Document) =
        when (val clazz = doc.getString(CLASS_FIELD)) {
            DOC_SEC_CLASS -> Either.left(convert(doc))
            DOC_SEC_TABLE_CLASS -> Either.right(toSectionTable(doc))
            else -> error("Expecting $clazz to be one of [$DOC_SEC_CLASS , $DOC_SEC_TABLE_CLASS]")
        }
