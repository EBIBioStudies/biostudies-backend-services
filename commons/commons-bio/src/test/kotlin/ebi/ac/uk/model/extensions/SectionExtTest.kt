...
class SectionExtTest {
    ...
    @Test
    fun `parent accession number`() {
        val subsection = Section("Subsection 1")
        subsection.parentAccNo = "SECT-001"

        assertThat(subsection.parentAccNo).isEqualTo("SECT-001")
        assertThat(subsection.attributes).hasSize(1)
        assertThat(subsection.attributes.first()).isEqualTo(Attribute(SectionFields.PARENT_ACC_NO, "SECT-001"))
    }
}
