...
fun Submission.section(
    type: String,
    block: Section.() -> Unit,
) = apply { section = Section(type).apply(block) }

fun submission(
    accNo: String = "",
    block: Submission.() -> Unit,
): Submission = Submission(accNo).apply(block)

fun section(
    type: String,
    block: Section.() -> Unit,
): Section = Section(type).apply(block)
...
fun FilesTable.file(
    name: String,
    block: BioFile.() -> Unit = {},
) = addRow(BioFile(name).apply(block))

fun SectionsTable.section(
    type: String,
    block: Section.() -> Unit = {},
) = addRow(Section(type).apply(block))

fun LinksTable.link(
    url: String,
    block: Link.() -> Unit = {},
) = addRow(Link(url).apply(block))
...
fun Section.file(
    name: String,
    block: BioFile.() -> Unit = {},
) = addFile((BioFile(name).apply(block)))

fun Section.section(
    type: String,
    block: Section.() -> Unit,
) = addSection((Section(type).apply(block)))

fun Section.link(
    url: String,
    block: Link.() -> Unit = {},
) = addLink((Link(url).apply(block)))

fun link(
    url: String,
    block: Link.() -> Unit = {},
) = Link(url).apply(block)
