...
internal class FileListTsvStreamDeserializer {
    ...
    private fun writeHeaderNames(firstFile: PageTabFile, writer: BufferedWriter) {
    ...
    private fun buildAttributes(fields: List<String>, headers: List<String>, index: Int): List<Attribute> {
        if (fields.size != headers.size)
            throw InvalidElementException("Error at row # $index. The row must have the same attributes as the header")
