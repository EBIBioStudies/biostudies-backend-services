...
class TranspilerCommandLineTest(
    ...
    fun setUp() {
        ...
        every { mockTranspiler.transpile("", listOf("colA", "colB"), "/path", "base", PlainTsv) } returns ""
    }
    ...
    fun `transpile template`() {
        ...
        verify(exactly = 1) { mockTranspiler.transpile("", listOf("colA", "colB"), "/path", "base", PlainTsv) }
    }

    @Test
    fun `transpiler exception`() {
        every { mockTranspiler.transpile("", listOf("colA", "colB"), "/path", "base", PlainTsv) } throws Exception("Some exception")
