...
import uk.ac.ebi.extended.serialization.constants.ExtSerializationFields.PROJECTS
...
class ExtSubmissionSerializer : JsonSerializer<ExtSubmission>() {
    ...
    override fun serialize(submission: ExtSubmission, gen: JsonGenerator, serializers: SerializerProvider) {
        ...
        gen.writeObjectField(PROJECTS, submission.projects)
    ...
    private fun getAccessTag(submission: ExtSubmission): List<ExtAccessTag> {
        val projects = submission.projects.map { ExtAccessTag(it.accNo) }.plus(ExtAccessTag(submission.owner))
