...
class ExtSectionSerializerTest(private val tempFolder: TemporaryFolder) {
    ...
    fun `serialize all in one`() {
        ...
        assertThat(testInstance.serialize(allInOneSection, SerializationProperties(includeFileListFiles = false))).isEqualToIgnoringWhitespace(expectedJson)
    }

    @Test
    fun `serialize all in one including file list files`() {
        val referencedFile = tempFolder.createFile("ref-file.txt")
        val sectionFile = tempFolder.createFile("section-file.txt")
        val sectionFilesTable = tempFolder.createFile("section-file-table.txt")
        ...
        val expectedJson = jsonObj {
            ...
            "fileList" to jsonObj {
                ...
                "files" to jsonArray(
                    jsonObj {
                        "fileName" to referencedFile.name
                        "filePath" to "folder/${referencedFile.name}"
                        "relPath" to "Files/folder/${referencedFile.name}"
                        "fullPath" to "root/Files/folder/${referencedFile.name}"
                        "file" to referencedFile.absolutePath
                        "attributes" to jsonArray()
                        "extType" to "nfsFile"
                        "type" to "file"
                        "size" to referencedFile.size()
                    }
                )
                "pageTabFiles" to jsonArray(
                    jsonObj {
        ...
        ExtSectionSerializer.parentAccNo = "S-BSST1"
        assertThat(testInstance.serialize(allInOneSection, SerializationProperties(includeFileListFiles = true))).isEqualToIgnoringWhitespace(expectedJson)
    }
}
